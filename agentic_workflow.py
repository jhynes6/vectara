...
class WorkflowTools:
    ...
    def generate_brief(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Save to Supabase DB (client_briefs table) - REQUIRED for chunking
            from supabase_brief_generator import save_brief_to_supabase_db
            save_brief_to_supabase_db(self.client_id, brief)
            logger.info(f"✅ Saved brief to client_briefs table for {self.client_id}")
            ...
            return {
                'success': True,
                'message': 'Brief generated and saved to database successfully',
    ...
    def chunk_client_brief(self) -> Dict[str, Any]:
        """Chunk client brief and save to document_chunks table"""
        logger.info("🔧 Tool: chunk_client_brief")
        
        try:
            # Import the inbox manager vector store
            sys.path.insert(0, str(Path(__file__).parent / 'inbox_manager'))
            from vector_store import VectorStore
            
            vs = VectorStore()
            
            # Save client brief chunks
            success = vs.save_client_brief_chunks(self.client_id)
            
            if success:
                self.state['brief_chunking_complete'] = True
                logger.info(f"✅ Successfully chunked client brief for {self.client_id}")
                return {
                    'success': True,
                    'message': f'Client brief chunked and saved to document_chunks table for {self.client_id}'
                }
            else:
                error_msg = f"Failed to chunk client brief for {self.client_id}"
                logger.error(error_msg)
                return {
                    'success': False,
                    'error': error_msg
            ...
            error_msg = f"Client brief chunking failed: {str(e)}"
            self.state['errors'].append(error_msg)
            logger.error(error_msg)
            return {
                'success': False,
...
5. **Brief Chunking Phase**: Chunk the client brief and save to document_chunks table for fast email handler access
...
IMPORTANT: You MUST run generate_brief BEFORE chunk_client_brief. The chunking step requires the brief to be saved to the client_briefs table first, which happens during brief generation.
...
Your response should be concise and actionable."""
        ...
        tools_spec = [
            ...
            },
            {
                "type": "function",
                "function": {
                    "name": "chunk_client_brief",
                    "description": "Chunk the client brief and save chunks to document_chunks table. MUST be run after generate_brief completes successfully (requires brief in client_briefs table).",
                    "parameters": {
                        "type": "object",
                        "properties": {},
                        "required": []
    ...
    async def execute_tool_call(self, tool_name: str, tool_args: Dict) -> Dict[str, Any]:
        ...
        tool_methods = {
            ...
            'chunk_client_brief': self.tools.chunk_client_brief,
...
4. Brief Generation
5. Brief Chunking (REQUIRED after brief generation)
...
Proceed with the workflow."""
            ...
            while iteration < max_iterations:
                ...
                if run.status == 'completed':
                    ...
                    # Re-check run status before submitting (it may have expired during tool execution)
                    run_check = self.openai_client.beta.threads.runs.retrieve(
                        thread_id=thread.id,
                        run_id=run.id
                    )
                    
                    if run_check.status == 'expired':
                        logger.error("❌ Run expired while executing tools. Cannot submit outputs.")
                        logger.info("💡 Tool execution took too long. Consider optimizing or using direct workflow instead.")
                        break
                    ...
                    try:
                        ...
                        if 'expired' in str(e).lower():
                            logger.error("❌ Run expired - cannot submit tool outputs")
                            break
                        raise
            ...
            logger.info(f"  {'✅' if final_state['upload_complete'] else '❌'} Supabase Upload: {'Complete' if final_state['upload_complete'] else 'Failed'}")
            logger.info(f"  {'✅' if final_state['brief_complete'] else '❌'} Brief Generation: {'Complete' if final_state['brief_complete'] else 'Failed'}")
            logger.info(f"  {'✅' if final_state['brief_chunking_complete'] else '❌'} Brief Chunking: {'Complete' if final_state['brief_chunking_complete'] else 'Failed'}")
            ...
            return {
                'success': all([
                    ...
                    final_state.get('brief_chunking_complete', True)  # Optional but recommended
                ]),
                'client_id': self.client_id,
