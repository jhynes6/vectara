...
class SupabaseVectorStore:
    ...
    def ensure_client_exists(self, client_id: str, primary_domain: str = None, 
        ...
        try:
            with conn.cursor() as cur:
                # Use client_id directly as text (no UUID conversion needed)
    ...
    def upload_document(self, 
                       ...
                       language: str = "eng",
                       document_id: str = None) -> Tuple[str, int]:
        """
        ...
        Args:
            ...
            document_id: Optional existing document ID (if provided, skips document creation)
        ...
        try:
            ...
            with conn.cursor() as cur:
                # If document_id is provided, use it (document already exists)
                if document_id:
                    logger.info(f"Using existing document ID: {document_id}")
                    # Verify the document exists and belongs to this client
                    cur.execute(
                        "SELECT id FROM documents WHERE id = %s AND client_id = %s",
                        (document_id, client_uuid)
                    )
                    if not cur.fetchone():
                        raise ValueError(f"Document {document_id} not found for client {client_id}")
                else:
                    ...
                    cur.execute(
                        ...
                        (client_id, source_type, content_type, title, uri, 
                         sha256, mime_type, language, metadata)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                        RETURNING id
                        """,
                        (client_uuid, source_type, content_type, title, uri,
                ...
                for idx, chunk in enumerate(chunks):
                    ...
                    chunk_data.append((
                        ...
                        embedding,
                        content_type  # Add content_type directly
                    ))
                ...
                execute_values(
                    ...
                    (document_id, client_id, chunk_index, content, content_sha256, 
                     token_count, embedding, content_type)
                    VALUES %s
                    """,
                    chunk_data
                )
    ...
    def query_documents(self, 
        ...
        try:
            ...
            use_semantic = bool(query and query.strip())
            ...
            with conn.cursor() as cur:
                ...
                if use_semantic:
                    ...
                    # Exact filter only (no semantic similarity); prefer first chunk per document
                    query_sql = """
                        SELECT 
                            dc.id,
                            dc.content,
                ...
                if content_type_filter:
                    query_sql += " AND d.content_type = %s"
                    params.append(content_type_filter)
                
                if source_type_filter:
                    query_sql += " AND d.source_type = %s"
    ...
    def list_documents(self, client_id: str, limit: int = 100) -> List[Dict[str, Any]]:
        """
        List all documents for a client with readable type names
        ...
        try:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    WHERE d.client_id = %s
                    GROUP BY d.id, d.title, d.uri, d.sha256, d.mime_type, d.language, d.metadata, d.ingested_at, d.source_type, d.content_type
                    ORDER BY d.ingested_at DESC
                    LIMIT %s
                    """,
                    (client_uuid, limit)
    ...
    def get_document(self, document_id: str) -> Dict[str, Any]:
        """
        Get a single document with readable type names
